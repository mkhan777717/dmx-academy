  ...
  html = html.replace(codeBlockRegex, (match, lang, codeContent) => {
    ...
    if (lang === "javascript" || lang === "js" || !lang) {
      ...
      highlighted = highlighted.replace(tokenRegex, (m, comment, string, keyword, builtin, number) => {
        ...
        if (string) return `<span class="text-rose-400">${string}</span>`;
        if (keyword) return `<span class="text-blue-400 font-bold">${keyword}</span>`;
        if (builtin) return `<span class="text-amber-400 font-semibold">${builtin}</span>`;
...
export default function CreateProblem() {
  ...
  // 2. Statement State (completely empty/clean by default)
  ...
  // 3. Templates State
  const [templateJS, setTemplateJS] = useState("// JavaScript Starter Code\nfunction solve(input) {\n  // Write your code here\n  return \"\";\n}");
  const [templatePython, setTemplatePython] = useState("# Python 3 Starter Code\ndef solve(input_data):\n    # Write your code here\n    pass");
  ...
  const handleLoadTemplate = (type) => {
    if (type === "fizzbuzz") {
      setTitle("FizzBuzz Challenge");
      setCode("FIZZBUZZ");
      setDifficulty("Easy");
      setCategory("Basic Logic");
      setTags("logic, math, conditionals");
      setDesc(`Write a program that prints the numbers from \`1\` to \`N\`. But for multiples of three print \`Fizz\` instead of the number and for the multiples of five print \`Buzz\`. For numbers which are multiples of both three and five print \`FizzBuzz\`.

### Examples
For \`N = 5\`, your output should list the values:
1
2
Fizz
4
...
For \`N = 15\`, the output should list \`FizzBuzz\` at index 15.

\`\`\`javascript
// Example check
solve(15);
\`\`\``);
      setInputFormat("A single integer `N` representing the upper limit.");
      setOutputFormat("Output numbers from 1 to `N` line by line, replacing multiples of 3 with `Fizz`, multiples of 5 with `Buzz`, and multiples of both with `FizzBuzz`.");
      setConstraints("1 <= N <= 1000");
      ...
      setCategory("Arrays / Pointers");
      setTags("arrays, implementation");
      setDesc(`Given an array \`A\` of size \`N\`, write a function that reverses the elements in-place and returns the space-separated result.

### Example
For array \`[1, 2, 3, 4, 5]\`, the output should reverse all elements:

\`\`\`javascript
[5, 4, 3, 2, 1]
\`\`\`

Explain the output: The first element becomes last, the second becomes second-to-last, and so on.`);
      setInputFormat("First line contains `N`, the size of array. Next line contains `N` space-separated integers.");
      setOutputFormat("Output the reversed elements as space-separated integers.");
  ...
  return (
    <div className="space-y-6">
      ...
      {success && (
        ...
        >
          🎉 Problem created and added to library successfully!
        </motion.div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
        ...
        <div className="lg:col-span-3">
          <div className="glass-panel p-6 sm:p-8 rounded-3xl shadow-sm space-y-6 min-h-[450px]">
            ...
            {/* Tab 2: Statement (Side-by-Side Editor & CodeChef Style Preview) */}
            {activeTab === "statement" && (
              <motion.div
                initial={{ opacity: 0, x: 10 }}
                animate={{ opacity: 1, x: 0 }}
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-100/50 dark:bg-slate-800/40 p-4 rounded-2xl border border-dashed border-slate-200 dark:border-slate-700">
                  <div className="space-y-0.5">
                    <h4 className="text-xs font-bold" style={{ color: "var(--text-primary)" }}>💡 Load Problem Templates</h4>
                    <p className="text-[10px]" style={{ color: "var(--text-secondary)" }}>
                      Need help or formatting inspiration? Load a completed template structure or clear the editor.
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <button
                      type="button"
                      onClick={() => handleLoadTemplate("fizzbuzz")}
                      className="px-3 py-1.5 rounded-xl bg-indigo-500/10 text-indigo-500 hover:bg-indigo-500/20 text-[10px] font-bold transition-all border border-indigo-500/20 cursor-pointer"
                    >
                      FizzBuzz (Easy)
                    </button>
                    <button
                      type="button"
                      onClick={() => handleLoadTemplate("reverse")}
                      className="px-3 py-1.5 rounded-xl bg-violet-500/10 text-violet-500 hover:bg-violet-500/20 text-[10px] font-bold transition-all border border-violet-500/20 cursor-pointer"
                    >
                      Reverse Array (Medium)
                    </button>
                    <button
                      type="button"
                      onClick={() => handleLoadTemplate("clear")}
                      className="px-3 py-1.5 rounded-xl bg-rose-500/10 text-rose-500 hover:bg-rose-500/20 text-[10px] font-bold transition-all border border-rose-500/20 cursor-pointer"
                    >
                      Clear All
                    </button>
                  </div>
                        ...
                        {showGuide && (
                          <div className="space-y-3 pt-2.5 text-[var(--text-secondary)] border-t border-indigo-500/10 transition-all">
                            ...
                            <div>
                              ...
                              <pre className="bg-slate-900 text-slate-300 p-2.5 rounded-lg font-mono text-[10px] mt-1 leading-relaxed shadow-inner">
                                {"```javascript\nwhile (condition) {\n  // your code here\n}\n```"}
                              </pre>
                            </div>
                            <div>
